...
{
	int r = ((*time_seed ^ 335231451) + id) % 3;
    return r;
}
...
void Game(int *x) {
	...
	if (win) {
		...
		*round_index += 1;
		...
		MutexLockBlocking(MUTEX_ADDR, 10, cpuId);
		xil_printf("P2: Am castigat runda\r\n");
		MutexUnlock(MUTEX_ADDR, 10, cpuId);
...
int main()
{
	...
	while (1) {
		...
		MboxReadBlocking(Q_0_if_0, x + 1, 1); // primeste x[1]
		...
		MboxWriteBlocking(Q_1_if_1, x + 2, 1); // trimite x[2]
		MboxReadBlocking(Q_0_if_0, x , 1); // primeste x[0]
		MboxWriteBlocking(Q_1_if_1, x + 1, 1); // trimite x[1]
