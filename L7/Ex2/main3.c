...
volatile u32* intc3_ier_addr = (u32*)(XPAR_AXI_INTC_3_BASEADDR + 0x8);
volatile u32* intc3_iar_addr = (u32*)(XPAR_AXI_INTC_3_BASEADDR + 0xC);
volatile u32* intc3_ivr_addr = (u32*)(XPAR_AXI_INTC_3_BASEADDR + 0x18);
volatile u32* intc3_mer_addr = (u32*)(XPAR_AXI_INTC_3_BASEADDR + 0x1C);
...
void Sw1_ISR() {
	...
	xil_printf("P3: 3 -> 0\r\n");
	MutexUnlock(MUTEX_ADDR, 0, XPAR_CPU_ID);MutexUnlock(MUTEX_ADDR, 0, XPAR_CPU_ID);

    interrupt_flag = 1;
// Clear the pending interrupt
    *intc3_iar_addr |= (1 << XPAR_AXI_INTC_0_SW_3_INTR);


}
void GlobalISR() { // Interrupt Service Routine
    u32 active_isr = *intc3_ivr_addr;
    switch (active_isr) {
...
// Set interrupt controller: enable Sw1 interrupts
    *intc3_ier_addr |= (1 << XPAR_AXI_INTC_0_SW_3_INTR);
    *intc3_mer_addr = 0x3;
// Enable microblaze interrupt
    ...
    while (1) {
        if (interrupt_flag) {
            ...
            // send interrupt to core 0
        	*intc0_isr_addr |= (1 << XPAR_AXI_INTC_0_SW_0_INTR);
